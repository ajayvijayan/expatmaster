 ...
 <div class="row p-1 ">
    <div class="col-12 pt-3">
      <div class="alert bg-gray btn-point" role="alert">
         <i class="fas fa-list-alt"></i>&nbsp; Moved Expats From IQ to HQ List
      </div> <!-- ./alert -->
    </div> <!-- ./col12 -->
  </div> <!-- ./row -->
  
  @if(Auth::user()->usertype == 'Lsg official')  
  <form action="{{ route('lsg.filtermovedIQtoHQlist') }}" name="housevisitlist" method="post">
  @elseif(Auth::user()->usertype == 'Data entry operator')   
  <form action="{{ route('deo.filtermovedIQtoHQlist') }}" name="housevisitlist" method="post">
  @elseif(Auth::user()->usertype == 'phcmo')   
  <form action="{{ route('phc.filtermovedIQtoHQlist') }}" name="housevisitlist" method="post">  
  @endif 
    @csrf
  <div class="row p-1 ">
    <div class="col-12 pt-3">
...
@if(isset($listdatas))
  <span class="eng_xxs text-darkslategray">
         <font class="text-danger">  Moved Expats to Institution Quarantine List : From {{ $frmdt }} To {{ $todt }}  </font>
         </span><br/>
