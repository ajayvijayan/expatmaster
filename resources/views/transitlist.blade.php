    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content modalevenrow">
            ...
            <div class="modal-body modalevenrow text-dark">
                ...
                <div class="row p-2 modalevenrow">
                    <div class="col-4 d-flex justify-content-end"> District
                    </div> <!-- ./col4 -->
                    <div class="col-8">
...
$("#master_table").DataTable({
    ...
    serverSide: true,
    ajax: {
        url: "{{ route('admin.transitpointlist') }}",
    },
    columns: [
    ...
    $("#transactionbutton").click(function(event){ 
      ...
      $.ajax({
       url: "{{ route('admin.transitpointcreate') }}",
       dataType:"json",
       success:function(data)
        {
            ...
            $('.modal-title').text('Add New Transit Point');
            $('#actionbutton').val('Save Details');
            $('#action').val('Add');
            $('#ajaxformresults').html('');
            $("#transactionmodal").modal('show');
     ...
     $('#ajaxmodalform').on('submit', function(event){
      ...
      if($('#name').val() == ''){
        ...
        $('#name').focus();
          ...
          if($('#action').val() == 'Add')
              action_url = "{{ route('admin.transitpointstore') }}";

          if($('#action').val() == 'Edit')
              action_url = "{{ route('admin.transitpointupdate') }}";
 ...
 $('#ok_button').click(function(){
    ...
    $.ajax({
            url:"/admin/transitpointdestroy/"+element_id,
            dataType:"json",
