 ...
 <div class="row p-1 ">
    <div class="col-12 pt-3">
      <div class="alert bg-gray btn-point" role="alert">
         <i class="fas fa-list-alt"></i>&nbsp; Moved Expats From HQ to IQ List
      </div> <!-- ./alert -->
    </div> <!-- ./col12 -->
  </div> <!-- ./row -->
  
  @if(Auth::user()->usertype == 'Lsg official')  
  <form action="{{ route('lsg.filtermovedHQtoIQlist') }}" name="housevisitlist" method="post">
  @elseif(Auth::user()->usertype == 'Data entry operator')   
  <form action="{{ route('deo.filtermovedHQtoIQlist') }}" name="housevisitlist" method="post">
    @elseif(Auth::user()->usertype == 'phcmo')   
  <form action="{{ route('phc.filtermovedHQtoIQlist') }}" name="housevisitlist" method="post">
  @endif 
...
@if(isset($listdatas))
  <span class="eng_xxs text-darkslategray">
         <font class="text-danger">  Moved Expats to Home Quarantine List : From {{ $frmdt }} To {{ $todt }}  </font>
         </span><br/>
